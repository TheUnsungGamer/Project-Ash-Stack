...
interface UseChatOptions {
  ...
  onVerityAudio?: (audioData: string) => void;
  onServitorAudio?: (audioData: string) => void;
}
...
export function useChat({
  ...
  const handleMessage = useCallback((data: {
    ...
    switch (data.type) {
      case "verity_text":
        // Verity has responded - update the assistant message
      ...
      case "verity_audio":
        // Verity TTS audio ready
        if (data.audio_data && onVerityAudioRef.current) {
          onVerityAudioRef.current(data.audio_data);
        }
        break;

      case "servitor_pending":
        // Servitor is computing
        setServitorPending(true);
        break;

      case "servitor_result": {
        // Servitor has flagged something
        setServitorPending(false);
        const servitorData: ServitorResult = {
        ...
        setServitorResult(servitorData);

        if (onServitorResultRef.current) {
          onServitorResultRef.current(servitorData);
        }

        if (data.audio_data && onServitorAudioRef.current) {
          onServitorAudioRef.current(data.audio_data);
        }
        break;
      }

      case "servitor_optimal":
        // Servitor approved - no action needed
        setServitorPending(false);
        console.log("[Ash] Servitor: OPTIMAL - confidence:", data.confidence);
        break;

      case "error":
        console.error(`[Ash] Error from ${data.source}:`, data.message);
        setErrorMessage(`${data.source}: ${data.message}`);
        setIsStreaming(false);
        setServitorPending(false);
        break;
